...
import { Download, Globe, Sun, Moon, Lock, Menu, X, BookOpen } from 'lucide-react';
...
const sectionLabels: Record<string, { zh: string; en: string }> = {
  ...
  skills: { zh: '技能与证书', en: 'Skills' },
  tools: { zh: 'Vibe Coding', en: 'Vibe Coding' },
  metrics: { zh: '数据指标', en: 'Metrics' },
};
...
export default function Sidebar({
  ...
  const navContent = (
    <>
      {/* Logo */}
      <a href="#" className="flex items-center gap-2.5 hover:opacity-80 transition mb-8">
      ...
      <nav className="flex-1 space-y-1">
        {sectionOrder.filter(id => sectionLabels[id]).map(id => (
          ...
          >
            <span className={`w-1.5 h-5 rounded-full flex-shrink-0 transition-colors ${
              activeSection === id ? 'bg-primary-500' : 'bg-transparent'
            }`} />
        ...
        <a
          href="/blog/"
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-200 transition-colors"
        >
      ...
      <div className="border-t border-gray-100 dark:border-gray-800 pt-4 space-y-1.5">
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 hover:bg-primary-100 dark:hover:bg-primary-900/40 transition"
        >
          <Download size={16} />
          {lang === 'zh' ? '导出简历' : 'Resume'}
        </button>
  ...
  return (
    <>
      ...
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-56 bg-white dark:bg-gray-950 border-r border-gray-100 dark:border-gray-800 flex-col p-5 overflow-y-auto z-50">
        {navContent}
      </aside>
    </>
  );
